-- Migration 027: Extend system_integrity_view with confidence-based checks
--
-- Adds 2 new checks (total: 10 rows):
--   I) low_confidence_happenings — published happenings with confidence_score < 50
--   J) tier_b_without_image_ratio — ratio of Tier B published happenings missing images
--
-- This is a CREATE OR REPLACE VIEW. All 8 original checks are preserved.
-- No data changes. No feed changes. Read-only diagnostics only.
...
--    Uses best_source (same priority as views) to determine source tier.
SELECT
